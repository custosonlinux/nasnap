...
- **Instant Recovery — Discard destroyed the migrated VM** — a completed session's Discard button stayed active and, since the FlexClone/temp storage were already gone after a successful Storage Migrate, running it destroyed the now-permanent VM instead of a temporary one. Completed sessions now show a **Remove** action that only clears the list entry.
...
- **Manifest lookup now also falls back to the live manifest tree, then to the VM's live PVE config** — a snapshot only ever "sees" NaSnap manifests that existed at the moment it was frozen; a VM not yet captured by NaSnap at that point (common for ONTAP-native/anti-ransomware snapshots) failed with "VM not found in snapshot manifest" even though a later manifest does know it. Restore, Clone, and Instant Recovery now also check the current live manifest tree, and — if the VM has never appeared in any NaSnap manifest at all — fall back to reading its disk layout straight from its current Proxmox config. Every fallback tier tried is logged to the job's Activity Log for diagnosis.
